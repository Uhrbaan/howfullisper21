...
# the RSSI is the signal strength so very useful to know aproximately the distance: 
    -30 dBm = very close
    -60 dBm = ~2–5 meters
    -90 dBm = near limit of detection

# SPIFFS = SPI Flash File System
    It's a way to save data directly to the internal flash memory of your ESP32 board (no SD card needed). You can write to files like on a computer:
    Create a text file
    Append lines (like MAC + timestamp)
    Read it later via serial
  Think of it like a tiny built-in USB stick.
